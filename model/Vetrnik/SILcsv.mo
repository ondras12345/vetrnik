within Vetrnik;
model SILcsv "software-in-the-loop simulation of the whole 'vetrnik' system with lisp control"
  ...
  Modelica.Blocks.Interfaces.RealInput vwind annotation(
    Placement(transformation(origin = {-90, 0}, extent = {{-20, -20}, {20, 20}}), iconTransformation(origin = {-84, 2}, extent = {{-20, -20}, {20, 20}})));
equation
...
// max(0, ...) is needed to prevent failing assert in C due to floating point errors.
    duty = vetrnikController(time, rpm, uFilter.y, max(0, iFilter2.y), windTurbine.vwind);
  end when;
